  ...
  try {
    ...
    if (details.courseId == null) {
      ...
      for (var i = 0; i < courses.length; i++) {
        staffMember = await accountsModel.findOne({
  ...
  try {
    ...
    if (details.academicId == null) {
      ...
      for (var i = 0; i < staff.length; i++) {
        var staffMemberDayOff = {
          academicId: staff[i].academicId,
          firstName: staff[i].firstName,
          lastName: staff[i].lastName,
          dayOff: staff[i].dayOff,
        }
      ...
      const staffMemberDayOff = {
        academicId: staff[0].academicId,
        firstName: staff[0].firstName,
        lastName: staff[0].lastName,
        dayOff: staff[0].dayOff,
      }
...
const viewCoursesCoverage = async (req, res) => {
  try {
    ...
    if (courseId !== undefined) {
      ...
      if (!courseFound) {
        return res.json({
          statusCode: 101,
          error:
            'this course either does not exist or is not in your depatment',
        })
      }
    } else {
      courses = await coursesModel.find({
    ...
    for (var i = 0; i < coursesSlots.length; i++) {
      ...
      var obj = {
        couseId: courses[i].courseId,
        coverage:
          coursesSlots[i].length === 0
            ? 100
            : (count / coursesSlots[i].length) * 100,
      }
      count = 0
